...
# Production build (runs tsc -b first)
...
- Hooks and CI checks:
  ...
  - Pre-commit hook: `.githooks/pre-commit` (Rust `cargo check` with `RUSTFLAGS=-D warnings`, optional `cargo clippy`, frontend lint with `--max-warnings=0`, and a frontend build).
  - Backend CI job: `backend/.github/workflows/rust.yml` runs `cargo test --all-features --verbose`.
...
- General
  ...
  - Do not change secrets or `.env` files. `backend/.env` exists and may contain local credentials - never commit secrets.
...
- Rust (backend)
  ...
  - Concurrency / async:
    - Use `tokio` async primitives. Prefer `async fn` and `.await` rather than blocking calls. Use `Arc` for shared state passed into routes.
  - Tests:
...
- TypeScript / React (frontend)
  ...
  - Formatting: repository does not include Prettier by default - follow semantic formatting with 2-space indentation and readable line lengths.
  - Linting: use `npm --prefix frontend run lint` (configured in `frontend/eslint.config.js`). Use `--max-warnings=0` in CI or before committing.
  - Imports:
    - Order imports: external packages (React, libs), absolute app imports (e.g. `@/`), then relative imports. Keep import paths concise.
    - Prefer named imports over default when library exports named items.
  - Naming:
    ...
    - Files: components in `frontend/src/components/` use PascalCase file names for default-exported components.
  ...
  - Types & safety:
    ...
    - Use the `@/` alias defined in `frontend/tsconfig.json` for app imports when it improves clarity.
...
- Tests & CI specifics
  ...
  - Frontend build in hooks runs `tsc -b && vite build` via `npm --prefix frontend run build`.

- Cursor / Copilot rules

  - No `.cursor/rules/`, `.cursorrules`, or `.github/copilot-instructions.md` were found at the time this file was updated. If they appear later, incorporate their guidance here.
...
References
- See `.githooks/pre-commit` for the exact pre-commit checks.
